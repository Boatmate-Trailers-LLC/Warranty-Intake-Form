...
- code path also parses: `application/json`

Important implementation nuance:
- the current dealer HTML form submits `multipart/form-data`
- the Worker only collects uploaded files from multipart submissions under `attachments`
- dealer submissions require at least one attachment
- therefore, while JSON is supported as a parse path, it is not a complete practical dealer submission contract in the current implementation
...
Current as-built behavior allows possible false-success conditions:
- ticket creation errors are caught into `ticketError`
- Worker can still return `ok: true`
- browser treats that as a successful submission

This pattern also applies to other non-fatal downstream failures:
- contact lookup/create failures may be logged without stopping all downstream flow
- attachment upload / note creation failures may still return `ok: true` with `attachmentsError`
- confirmation email failures may still return `ok: true` with `emailStatus = failed`

As a result, browser-visible success currently means:
- request parsed successfully
- authoritative validation passed
- claim number was generated
- Worker completed response flow

It does not strictly guarantee that every downstream HubSpot/email side effect completed successfully.
...
- `category` is client-submitted in the current form contract, but the Worker defensively defaults it to `Warranty` when omitted or blank
